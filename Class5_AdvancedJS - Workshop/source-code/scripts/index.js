$(document).ready(() => {
  ...
  function Person(name, height, mass, spiciesName, birthYear, appearances) {
    this.name = name;
    this.height = height;
    this.mass = mass;
  ...
  function Ship(name, model, manufacturer, cost, peopleCapacity, shipClass) {
    this.name = name;
    this.model = model;
...
printAllStudents(sedc.filter(s => s.name.length > 5));

//
let studentsUl = document.getElementById("students");
let filteredStudents = sedc
  .filter(s => s.name[0] === "M")
  .map(s => `${s.name} ${s.lastName}`);
// s = element (objekt). Sekoj objekt vo nizata e element (s).
